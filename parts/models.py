...
class Fixing(models.Model):
    ...
    fixing_category = models.ForeignKey(FixingCategory, on_delete=models.SET_NULL,
                                        null=True, related_name='fixing_cat')
    fixing_metric = models.ForeignKey(FixingMetric, on_delete=models.SET_NULL, null=True, related_name='fixing_met')
    fixing_detail = models.CharField(max_length=50, null=True)
    fixing_finish = models.ForeignKey(FixingFinish, on_delete=models.SET_NULL, null=True, related_name='fixing_fin')
    part = models.OneToOneField(Part, on_delete=models.CASCADE, null=True, related_name='fixing_part')
